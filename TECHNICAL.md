...
2. File list shows with individual Remove buttons; each file carries a stable numeric `_id` (from a `fileIdRef` counter) so Remove works correctly even when multiple mobile photos share the same generic filename (`image.jpg`)
3. "Extract Data with AI" button is pressed — a sliding indeterminate progress bar appears below it for the duration of the scan; for multi-file batches the button also shows a counter (`Extracting 2 of 5…`)
4. Results cards appear for review; fields with validation errors are highlighted in red
